...
class DeedTransferForm extends React.Component {
	...
	async checkNavigationState() {
		...
		if (app_nav_target && (app_nav_target.route == 'deed') && (app_nav_target.reached == false)) {
			...
			if (dataobj && (dataobj.type === 'deed') && (dataobj.treated !== true)) {
				...
				registration_text += ' ' + mvcmyquote.formatDate(registration_time, 'YYYY-mm-dd HH:MM:SS') + '.';
	...
	async onTransfer() {
		...
		try {
			// check address
			let validaddress = await mvcmyquote.isValidAddress(rootsessionuuid, toaddress)
			.catch(err => {
			...
			let _feelevel = await mvcmyquote.getRecommendedFeeLevel(rootsessionuuid, walletuuid, deedcard.uuid, tx_fee);

			var canspend = await mvcmyquote.canCompleteTransaction(rootsessionuuid, walletuuid, deedcard.uuid, tx_fee, _feelevel).catch(err => {});
			...
			const txhash = mvcmyquote.transferDeed(rootsessionuuid, walletuuid, currency.uuid, minter, deed, toaddress, _feelevel)
			.catch(err => {
				console.log('error in DeedTransferForm.onTransfer: ' + err);
			});
