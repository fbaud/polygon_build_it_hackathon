...
class ClauseView extends React.Component {
	...
	componentDidMount() {
		...
		let registration_text = mvcmyquote.t('This clause has been registered.');
		
		let message_text = mvcmyquote.t('Clause signed with:');
	
		let sharelinkmessage = mvcmyquote.t('Send this link to share this clause:');
		...
		if (app_nav_target && (app_nav_target.route == 'clause') && (app_nav_target.reached == false)) {
			...
			if (dataobj && (dataobj.type === 'clause') && (dataobj.treated !== true)) {
				...
				if (deed) {
					...
					let registration_text = mvcmyquote.t('This clause has been registered on');
