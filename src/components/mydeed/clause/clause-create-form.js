...
class ClauseCreateForm extends React.Component {
		...
		if (this.state.signingkey != prevState.signingkey) {
			...
			if ( (currency) && (currency.uuid)) {
				...
				.then(card => {
					...
					return mvcmyquote.isCardOwningDeed(rootsessionuuid, walletuuid, currencyuuid, newcard.uuid, this.minter, this.deed);
				})
				.then(owning => {
					if (owning) {
						isOwner = true;
					...
					return mvcmyquote.getCurrencyPosition(rootsessionuuid, walletuuid, currencyuuid, deedcard.uuid);
				})
				.then((pos) => {
					position = pos;
				...
				.then((pos_int) => {
					...
					return mvcmyquote.formatCurrencyAmount(rootsessionuuid, currencyuuid, position);
				})
				.then((pos_string) => {
	...
	async checkNavigationState() {
		...
		if (app_nav_target && (app_nav_target.route == 'clause') && (app_nav_target.reached == false)) {
			...
			if (deed) {
				...
				const cur = await mvcmyquote.getCurrencyFromUUID(rootsessionuuid, currencyuuid)
				.catch(err => {});
				...
				// get the deed card
				deedcard = await mvcmyquote.getDeedOwningCard(rootsessionuuid, walletuuid, currencyuuid, minter, deed)
				.catch(err => {
				...
				if (deedcard) {
					isOwner = true;
					let pos = await mvcmyquote.getCurrencyPosition(rootsessionuuid, walletuuid, currencyuuid, deedcard.uuid);
			
					if (pos !== undefined) {
						deedcard_balance_int = await pos.toInteger();
						deedcard_balance_string = await mvcmyquote.formatCurrencyAmount(rootsessionuuid, currencyuuid, pos);
					}
	...
	async onSubmit() {
		...
		try {
			...
			var feelevel = await mvcmyquote.getRecommendedFeeLevel(rootsessionuuid, walletuuid, deedcard.uuid, tx_fee);

			var canspend = await mvcmyquote.canCompleteTransaction(rootsessionuuid, walletuuid, deedcard.uuid, tx_fee, feelevel).catch(err => {});
			...
			article_clause.signature = await mvcmyquote.signString(rootsessionuuid, walletuuid, deedcard.uuid, JSON.stringify(article_clause));
			article_clause.signer = deedcard.address;
			const clause_txhash = await mvcmyquote.registerClause(rootsessionuuid, walletuuid, currency.uuid, minter, deed, article_clause, feelevel)
			.catch(err => {
				console.log('error in ClauseCreateForm.onSubmit: ' + err);
			});
